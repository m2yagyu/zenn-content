---
title: "Colabのセル3つで作るLLMチャットボット：Hugging Face推論APIの最小構成"
emoji: "🤖"
type: "tech" # tech: 技術記事 / idea: アイデア
topics: ["生成ai", "chatgpt", "machinelearning", "huggingface", "colab"]
published: true
---

この記事を読むと、Google Colabのセル3つだけで、Hugging Faceの推論API経由でLLMチャットボットを動かせるようになります。モデルのローカルダウンロードもGPUも課金も不要で、トークンの安全な受け渡しから会話履歴の持たせ方までを最小構成で確認できます。
...
この記事は「生成AIの中身を物理から読む」シリーズの 1 本目です。

次に読む → [LLMのtemperatureは本当に温度だった](https://zenn.dev/m2yagyu/articles/llm-temperature-boltzmann) — softmaxが統計力学のボルツマン分布そのものだと測って確かめる

:::details シリーズ全7本

1. **Colabのセル3つで作るLLMチャットボット**（この記事）— まずLLMを自分の手で動かす
