---
title: "784次元にしても式は変わらないのか"
---

前章の実験は2次元の点でした。「トイモデルだから綺麗に動いただけでは」と思うのが自然です。

この章では同じ仕組みを $28 \times 28 = 784$ 次元の手書き数字画像に持っていきます。変えるのは2箇所だけ——ネットワークをMLPからU-Netにすることと、学習データを画像にすること。**forward / reverse の数式にも、物理との対応にも、一切手を入れません。**

それで動くなら、前章で見た対応は次元の低さに依存していなかったことになります。
...
この章の元記事: [diffusion-model-mnist-unet](https://zenn.dev/m2yagyu/articles/diffusion-model-mnist-unet)
